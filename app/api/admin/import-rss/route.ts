...
function check(req: Request): boolean {
  ...
  return h.slice(prefix.length).trim()===process.env.CRON_SECRET?.trim();
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const resp = await fetch("https://feeds.feedburner.com/OLD-WordOfTheDay",{headers:{"User-Agent":"Mozilla/5.0"},signal:AbortSignal.timeout(15000)});
    if (!resp.ok) return NextResponse.json({error:"Feed status: "+resp.status},{status:502});
    ...
    if (!items.length) return NextResponse.json({error:"No entries found"},{status:400});
    const {error} = await sb.from("daily_words").upsert(items,{onConflict:"fetched_date"});
